...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			$("#AddRow").click(function(){		
				...
				tr.find("td").get(2).innerHTML="<input  name=claimVoucherDetail["+j+"].desc  id=desc"+j+" type=hidden value="+desc+" />"+desc;		
	...
	<div class="action  divaction">
		...
		<div class="pages">
			<form action="updateClaim.action" name="claimForm" method="post"> 
			...
			<input type="hidden" id="claimId" name="claimVoucher.id" value="<s:property value="claimVoucher.id"/>"/>
				<table width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
                  ...
                  <tr>
                    <td>总金额：
                    <input type="text"  name="claimVoucher.totalAccount" id="totalAccount" readonly="true"  value="<s:property value='claimVoucher.totalAccount'/>"/></td>
				...
				<table id="myTable" width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
					...
					<s:iterator value="claimVoucher.bizClaimVoucherDetails" begin="0" status="s">
					<tr>
						<td>
							 <input type="hidden" id="id" name="detailList[${s.index}].id" value="<s:property value='id'/>"/>
							 <input type="hidden" id="item" name="detailList[${s.index}].item" value="<s:property value='item'/>"/>
							<s:property value="item"/></td>
						<td>
							 <input type="hidden" id="account" name="detailList[${s.index}].account" value="<s:property value='account'/>"/>
							<s:property value="account"/></td>
						<td>
							 <input type="hidden" id="des" name="detailList[${s.index}].des" value="<s:property value='des'/>"/>
				...
				<table>
					<tr>
						<td>*事由：</td>
						<td colspan="3">
						<textarea name="claimVoucher.event"  id="event" rows="5" cols="66" >
						<s:property value='claimVoucher.event'/>
						</textarea>
						</td>
					</tr>
